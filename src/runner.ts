...
export const run = async (config: IConfigInterface, output = console.log) => {
  ...
  const filterUsedInModule =
    config.unusedInModule !== undefined
      ? presented.filter((file) => !file.includes(USED_IN_MODULE))
      : presented;
  const filterIgnored =
    config.ignore !== undefined
      ? filterUsedInModule.filter((file) => !file.match(config.ignore!))
      : filterUsedInModule;
  ...
  if (config.performance) {
    ...
    output(
      `   Parallel processing: ${config.parallel ? "Enabled" : "Disabled"}`,
    );
  }
